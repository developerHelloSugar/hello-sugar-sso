...
const SSO_SERVICE_URL = process.env.SSO_SERVICE_URL || 'http://localhost:3002';
  ...
  try {
    ...
    if (response.ok && data.success) {
      ...
      return req.session.save((err) => {
        if (err) {
          console.error('[website1] Session save failed during token login:', err);
          return res.sendFile(path.join(__dirname, 'public', 'sso-login.html'));
        }
    ...
    console.error('[website1] Error during token login:', error);
    return res.sendFile(path.join(__dirname, 'public', 'sso-login.html'));
  }
});
...
app.post('/auth/generate-sso-for-site', async (req, res) => {
  ...
  if (!req.session.user) {
    console.log('[website1] User not authenticated on website1');
  ...
  try {
    ...
    if (response.ok) {
      console.log('[website1] SSO token generated for cross-site:', data.token);
      res.json(data);
    } else {
      console.error('[website1] Failed to generate SSO token:', data.error);
...
const PORT = process.env.WEBSITE1_PORT || 3000;
app.listen(PORT, () => {
